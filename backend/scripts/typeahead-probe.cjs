...
(async () => {
  ...
  const trouserKinds = (trousers.groups ?? []).flatMap((g) => (g.kinds ?? []).map((k) => ({ ...k, group: g.category })));
  check('a product type like trousers is found in every category that sells it', trouserKinds.length >= 2 && trouserKinds.every((k) => /trouser/i.test(k.name)) && new Set(trouserKinds.map((k) => k.category)).size >= 2 && (trousers.products ?? 0) === 0, trouserKinds.map((k) => `${k.name}@${k.category}`).join(','));
  const underwear = (await search('underwear')).groups?.flatMap((g) => g.kinds ?? []) ?? [];
  check('so is underwear, shirts, sofas, fridges: everything sellable, not just electronics', underwear.some((k) => k.name === 'Underwear') && ((await search('shirt')).groups ?? []).some((g) => (g.kinds ?? []).some((k) => /shirt/i.test(k.name))) && ((await search('sofa')).groups ?? []).some((g) => (g.kinds ?? []).some((k) => /sofa/i.test(k.name))) && ((await search('fridge')).groups ?? []).some((g) => (g.kinds ?? []).some((k) => /fridge/i.test(k.name))));
  const lg = await search('lg');
  const lgCats = (lg.groups ?? []).filter((g) => (g.brands ?? []).some((b) => b.brand === 'LG')).map((g) => g.category);
  check('a brand known in many categories fans out to each: LG in appliances and TVs', lgCats.includes('appliances') && lgCats.includes('tv-audio'), lgCats.join(','));
  const lgProducts = flat(lg);
  check('and lists its catalogue lines too (LG OLED TVs)', lgProducts.some((p) => /OLED C/.test(p.family ?? '')));
  const airForce = flat(await search('air force'));
  const af = airForce.find((p) => p.model === 'Air Force 1 Low');
  check('shoes are catalogued as perennial models: Nike Air Force 1 stays current', !!af && af.lifecycle === 'current' && af.category === 'shoes');
  const afLookup = await call('GET', '/knowledge/product?brand=Nike&model=Air%20Force%201%20Low');
  check('and carries shoe sizes 36 to 46 as official options', afLookup.body.known && ['36', '42', '46'].every((v) => afLookup.body.product.variants.sizes.includes(v)));
  const oldMac = flat(await search('macbook pro 2017'));
  check('older laptops are known for the used market: MacBook Pro 2017 is discontinued', oldMac.some((p) => /\(2017\)/.test(p.model) && p.lifecycle === 'discontinued'));
  const oldPhone = flat(await search('iphone x'));
  check('older phones too: iPhone X (2017) is discontinued', oldPhone.some((p) => p.model === 'iPhone X' && p.lifecycle === 'discontinued'));
  const boda = flat(await search('boxer'));
  check('boda models are perennial and never force Used', boda.some((p) => /Boxer BM 150/.test(p.model) && p.lifecycle === 'current'));
  const oil = await call('GET', '/knowledge/form?category=groceries&kind=Cooking%20oil');
  check('cooking oil asks for sizes in litres, not colours or a model', oil.body.kind?.name === 'Cooking oil' && oil.body.versionType === 'Size' && oil.body.versions.includes('5L') && oil.body.traits.colours === false && oil.body.traits.model === false, JSON.stringify(oil.body.traits));
  const towel = await call('GET', '/knowledge/form?category=home&kind=Towels');
  check('towels ask for size and material, knives for type and material', towel.body.versions.includes('Bath towel') && towel.body.attributes.some((a) => a.key === 'material') && (await call('GET', '/knowledge/form?category=kitchen&kind=Knives')).body.versionType === 'Type');
  const phoneTraits = (await call('GET', '/knowledge/form?category=phones&kind=Smartphones')).body.traits;
  check('phones keep brand, model and colours', phoneTraits.colours && phoneTraits.brand && phoneTraits.model);
  const jersey = await call('GET', '/knowledge/form?category=sports&kind=Jerseys');
  const club = jersey.body.attributes.find((a) => a.key === 'club');
  check('jerseys offer leagues and the clubs in each, most famous first', !!club && club.groups?.[0]?.label === 'Premier League' && club.groups[0].values.includes('Manchester United') && club.groups.some((g) => g.label === 'Uganda Premier League' && g.values.includes('Vipers SC')) && club.groupKey === 'league');
  check('jerseys also offer kit and season', jersey.body.attributes.some((a) => a.key === 'kit' && a.options.includes('Home')) && jersey.body.attributes.some((a) => a.key === 'season'));
  const united = await search('manchester united');
  const unitedHit = (united.groups ?? []).flatMap((g) => g.options ?? []).find((o) => o.value === 'Manchester United');
  check('typing a club name finds it as a jersey option with its league', !!unitedHit && unitedHit.group === 'Premier League' && unitedHit.kind === 'Jerseys' && unitedHit.groupKey === 'league');
  const soap = (await search('soap')).groups?.flatMap((g) => g.kinds ?? []) ?? [];
  const towelKind = (await search('towel')).groups?.flatMap((g) => g.kinds ?? []) ?? [];
  const handkerchief = (await search('handkerchief')).groups?.flatMap((g) => g.kinds ?? []) ?? [];
  check('everyday things resolve too: soap, towels, handkerchiefs, printing ink', soap.some((k) => k.name === 'Soap') && towelKind.some((k) => k.name === 'Towels') && handkerchief.some((k) => k.name === 'Handkerchiefs') && ((await search('printing ink')).groups ?? []).some((g) => (g.kinds ?? []).some((k) => /ink/i.test(k.name))));
  const ex = await call('GET', '/knowledge/examples');
  check('the name field gets rotating examples from the server', ex.status === 200 && ex.body.examples.length >= 10 && ex.body.examples.includes('cooking oil'));
  const fridgeBrand = (await search('hisense')).groups ?? [];
  check('Hisense fans out to TVs and appliances', fridgeBrand.some((g) => g.category === 'appliances' && (g.brands ?? []).length) && fridgeBrand.some((g) => g.category === 'tv-audio'));
  ...
  await pool.query(`DELETE FROM canonical_spec_sources WHERE product_id = $1 AND source_type = 'admin'`, [old.body.product.id]);
  await pool.query(`UPDATE canonical_specs cs SET value = src.value, value_norm = src.value_norm, confidence = 0.85, status = 'verified', locked_by_admin = FALSE, source_count = 1, best_tier = 1, updated_at = now()
                      FROM canonical_spec_sources src WHERE src.product_id = cs.product_id AND src.attribute_key = cs.attribute_key AND src.source_type = 'catalogue' AND cs.product_id = $1`, [old.body.product.id]);
  await pool.query(`DELETE FROM canonical_specs cs WHERE cs.product_id = $1 AND NOT EXISTS (SELECT 1 FROM canonical_spec_sources src WHERE src.product_id = cs.product_id AND src.attribute_key = cs.attribute_key)`, [old.body.product.id]);
