...
export function CheckoutPage() {
  ...
  const [placed, setPlaced] = useState<{ id: string; order_number: number; total_ugx: number; storeName?: string }[] | null>(null);
  ...
  useEffect(() => {
    let cancelled = false;
    Promise.all(cart.byStore.map((g) => api.get(`/marketplace/stores/${g.storeSlug}?limit=1`).then((r) => [g.storeId, Number(r.data.store.delivery_fee_ugx)] as const).catch(() => [g.storeId, 0] as const)))
      .then((pairs) => { if (!cancelled) setFees(Object.fromEntries(pairs)); });
    return () => { cancelled = true; };
  }, [cart.byStore]);

  const deliveryTotal = cart.byStore.reduce((s, g) => s + (fees[g.storeId] ?? 0), 0);
  ...
  async function placeOrder() {
    if (!addressId) { push('Choose where to deliver', 'error'); return; }
    setPlacing(true);
    try {
    ...
    return (
      <div className="mx-auto max-w-2xl px-4 py-12 sm:px-6">
        <div className="text-center">
          ...
          <p className="mt-2 text-body text-ink-2">{placed.length === 1 ? 'The store will confirm shortly and bring it to you.' : `${placed.length} stores will each confirm and deliver separately.`}</p>
        </div>
            ...
            <Link key={o.id} to={`/app/purchases/${o.id}`} className="block">
              <Card padding="md" hover>
                <div className="flex items-center justify-between gap-3">
                  <div><p className="font-medium text-ink">Order #{o.order_number}</p><p className="text-caption text-ink-3">Pay {formatUgx(o.total_ugx)} on delivery</p></div>
    ...
    <div className="mx-auto max-w-4xl px-4 pb-24 pt-4 sm:px-6 md:pb-16">
      ...
      <div className="grid gap-5 md:grid-cols-[1fr_320px]">
        <div className="flex flex-col gap-4">
          ...
          <Card padding="lg">
            ...
            {cart.byStore.map((g) => (
              <div key={g.storeId} className="mt-3 border-t border-line pt-3 first:mt-2">
                <div className="flex items-center justify-between text-sm"><span className="font-medium text-ink">{g.storeName}</span><span className="text-ink-3">Delivery {fees[g.storeId] === undefined ? '…' : formatUgx(fees[g.storeId])}</span></div>
          ...
          <Card padding="lg">
            <Textarea label="Note for the store (optional)" placeholder="Gate colour, best time to deliver…" value={notes} onChange={(e) => setNotes(e.target.value)} maxLength={500} rows={3} />
          </Card>
        </div>

        <div className="md:sticky md:top-20 md:self-start">
          <Card padding="lg">
            <h2 className="font-display text-h3 font-medium text-brand-green-deep">To pay on delivery</h2>
            <dl className="mt-3 flex flex-col gap-2 text-sm">
              <div className="flex justify-between"><dt className="text-ink-2">Items</dt><dd>{formatUgx(cart.subtotal)}</dd></div>
              <div className="flex justify-between"><dt className="text-ink-2">Delivery</dt><dd>{formatUgx(deliveryTotal)}</dd></div>
            ...
            <p className="mt-3 text-caption text-ink-3">Cash on delivery. The store confirms first, and you can cancel until they do.</p>
            <Button size="lg" fullWidth className="mt-4 hidden md:flex" onClick={placeOrder} disabled={placing || !addressId}>{placing ? 'Placing…' : 'Place order'}</Button>
          </Card>
        </div>
      </div>
      ...
      <div className="fixed inset-x-0 bottom-[calc(var(--duka-nav-height,0px))] z-30 border-t border-line bg-surface p-3 md:hidden" style={{ paddingBottom: 'calc(12px + env(safe-area-inset-bottom))' }}>
        <div className="flex items-center gap-3">
          <div><p className="text-caption text-ink-3">Pay on delivery</p><p className="font-display text-lg font-semibold text-brand-green-deep">{formatUgx(total)}</p></div>
          <Button size="lg" className="ml-auto flex-1" onClick={placeOrder} disabled={placing || !addressId}>{placing ? 'Placing…' : 'Place order'}</Button>
        </div>
      </div>
    </div>
  );
